...
res_compares <- res_summary_wide %>%
  mutate(counter_high_mn_diff = scn_mn_Counterfactual-scn_mn_high,
         counter_high_se_diff = sqrt(scn_se_Counterfactual^2+scn_se_high^2),
         counter_all_mn_diff = scn_mn_Counterfactual-scn_mn_all,
         counter_all_se_diff = sqrt(scn_se_Counterfactual^2+scn_se_all^2),
         
         high_all_mn_diff = scn_mn_high-scn_mn_all,
         high_all_se_diff = sqrt(scn_se_high^2+scn_se_all^2),
         
         counter_high_mn_rel = scn_mn_high/scn_mn_Counterfactual,
         counter_high_se_rel = se_ratio(scn_mn_high, scn_mn_Counterfactual,scn_se_high, scn_se_Counterfactual),
         counter_all_mn_rel = scn_mn_all/scn_mn_Counterfactual,
         counter_all_se_rel = se_ratio(scn_mn_all, scn_mn_Counterfactual,scn_se_all, scn_se_Counterfactual),
         
         high_all_mn_rel = scn_mn_all/scn_mn_high,
         high_all_se_rel = se_ratio(scn_mn_all, scn_mn_high, scn_se_all, scn_se_high)) %>%
  
  select(model_name, target, location, vax_tmg,
         counter_high_mn_diff, counter_all_mn_diff, counter_high_se_diff, counter_all_se_diff, high_all_mn_diff, high_all_se_diff,
         counter_high_mn_rel, counter_all_mn_rel, counter_high_se_rel, counter_all_se_rel, high_all_mn_rel, high_all_se_rel)

res_compares_lng <- res_compares %>%
  pivot_longer(cols=c(counter_high_mn_diff, counter_high_se_diff,
                      counter_all_mn_diff, counter_all_se_diff,
                      high_all_mn_diff, high_all_se_diff,
                      counter_high_mn_rel, counter_high_se_rel,
                      counter_all_mn_rel, counter_all_se_rel,
                      high_all_mn_rel, high_all_se_rel)) %>%
  mutate(measure=ifelse(stringr::str_detect(name,"mn"),"mn","se"),
         diff_rel = ifelse(stringr::str_detect(name,"diff"),"diff","rel"),
         compare=ifelse(stringr::str_detect(name,"counter_all"), "All booster vs Counterfactual", 
                        ifelse(stringr::str_detect(name,"counter_high"), "High-risk booster vs Counterfactual", 
                               "All vs High-risk booster")),
...
# ##### CHECK
# res_summary_wide_m %>% filter(model_name == "UTA-ImmunoSEIRS" & target == "inc hosp")
# 
# res_summary_wide_m %>% filter(model_name == "UTA-ImmunoSEIRS" & target == "inc hosp" & location == "02") %>%
#   mutate(AC_abs_diff = scA-scC, AE_abs_diff = scA-scE, CE_abs_diff = scC-scE) -> df
# 
# df %>%  dplyr::select(AC_abs_diff, AE_abs_diff, CE_abs_diff) %>% colMeans()
# apply(df %>% dplyr::select(AC_abs_diff, AE_abs_diff, CE_abs_diff), 2, sd)
# 
# compute_pairwise_diff <- function(column_data) {
#   combinations <- combn(length(column_data), 2)
#   pairwise_diff <- apply(combinations, 2, function(index) {
#     abs(column_data[index[1]] - column_data[index[2]])
#   })
#   return(pairwise_diff)
# }
# 
# c(mean(compute_pairwise_diff(df$scA)),
#   mean(compute_pairwise_diff(df$scC)),
#   mean(compute_pairwise_diff(df$scE)))
# 
# c(sd(compute_pairwise_diff(df$scA)),
#   sd(compute_pairwise_diff(df$scC)),
#   sd(compute_pairwise_diff(df$scE)))
...
res_compares_m <- res_summary_wide_m %>%
  ...
  dplyr::summarize(check=n(),
                   ...
                   BD_abs_diff_se = sd(BD_abs_diff, na.rm = TRUE)/sqrt(sum(!is.na(BD_abs_diff))),
                   CE_abs_diff_se = sd(CE_abs_diff, na.rm = TRUE)/sqrt(sum(!is.na(CE_abs_diff))),
                   
                   BD_rel_diff_se = sd(BD_rel_diff, na.rm = TRUE)/sqrt(sum(!is.na(BD_rel_diff))),
                   CE_rel_diff_se = sd(CE_rel_diff, na.rm = TRUE)/sqrt(sum(!is.na(CE_rel_diff))),
                   ...
                   BD_abs_diff_mn = mean(BD_abs_diff, na.rm = TRUE),
                   CE_abs_diff_mn = mean(CE_abs_diff, na.rm = TRUE),
                   
                   BD_rel_diff_mn = mean(BD_rel_diff, na.rm = TRUE),
                   CE_rel_diff_mn = mean(CE_rel_diff, na.rm = TRUE))




res_compares_m_lng <- res_compares_m %>%
  pivot_longer(cols=c(counter_B_abs_diff_se:CE_rel_diff_mn)) %>%
  mutate(measure=ifelse(stringr::str_detect(name,"se"),"se","mn"),
         diff_rel = ifelse(stringr::str_detect(name,"abs"),"diff","rel"),
         vax_tmg = ifelse(stringr::str_detect(name,"B|D"), "Classic Immunization","Early Immunization"),
         compare=ifelse(stringr::str_detect(name,"counter_D|counter_E"), 
                        "All booster vs Counterfactual", 
                        ifelse(stringr::str_detect(name,"counter_B|counter_C"), 
                               "High-risk booster vs Counterfactual", 
                               "All vs High-risk booster")),
         target_labs = ifelse(stringr::str_detect(target, "inc death"), "Deaths", "Hospitalizations")) %>%
  select(-name,-check) %>%
...
#       by = c("model_name", "location", "target", "vax_tmg", "diff_rel", "compare", "target_labs"), all=TRUE)

res_compares_m_lng -> res_compares_lng

res_compares_lng <- res_compares_lng %>% filter(location==loc_select)
...
for (vax_timing in c("Classic Immunization", "Early Immunization")) {
  
  vax_timing_lab <- tolower(str_split_1(vax_timing, " ")[1])
  
  png(filename = file.path(plot_dir, 
                           paste0(meta_name, '_abs_', vax_timing_lab, '_', gsub("-","_", age_group_tmp), loc_subname, '.png')), 
  ...
  for (comparison in c("All booster vs Counterfactual", "High-risk booster vs Counterfactual", "All vs High-risk booster")) {
    mdls[[vax_timing]][[comparison]] <- list()
    figs[[vax_timing]][[comparison]] <- list()
    for (tgt in tgt_vec) {
      ...
      mdls[[vax_timing]][[comparison]][[tgt]] <- meta
      figs[[vax_timing]][[comparison]][[tgt]] <- invisible(metafor::forest(meta,
                                                                           digits=0,
                                                                           col=outcome_colors[outcome_col_i],
                                                                           cex=cex_val,
                                                                           header=T,
                                                                           refline=refline,
                                                                           xlab=sprintf("%s (Averted) - %s, %s Immunization%s",
                                                                                        tgt, comparison, vax_timing, age_text_snip)))
      meta_res_abs <- meta_res_abs %>% bind_rows( 
        tibble(comp = comparison, vax_tmg = vax_timing, target = tgt, 
...
meta_res_abs %>% 
  mutate(vax_tmg=case_when(vax_tmg==c("Classic Immunization")~c("Classic timing"), 
                           vax_tmg==c("Early Immunization")~c("Early timing"))) -> meta_res_abs_plot



# deaths_meta_fig <- meta_res_abs %>% 
#   mutate(vax_tmg = paste0(vax_tmg, " Immunization")) %>%
#   as_tibble() %>%
#   filter(target == "Deaths") %>%
#   ggplot(aes(y = comp)) +
#   geom_errorbarh(aes(xmin = lb, xmax = ub), height = .25) + 
#   geom_point(aes(x = mean), size = 3, pch = 15) +
#   facet_grid(vax_tmg ~ target) +
...
#                    gp=gpar(fontsize=18))
# 
# y.grob <- textGrob("Vaccination target comparison", 
#                    gp=gpar(fontsize=18), rot=90)
# 
# title <- textGrob(paste0("Individuals aged ", age_group_label), gp=gpar(fontsize=18), hjust = 1.62)
# 
# png(filename = file.path(plot_dir, 
#                          paste0(meta_name, '_abs_pooled_', gsub("-","_", age_group_tmp), loc_subname, '.png')),
#     width = 12,
#     height = 6,
#     units = 'in',
#     res = 600)
...
for (vax_timing in c("Classic Immunization", "Early Immunization")) {
  ...
  filename_ <- file.path(plot_dir, paste0(meta_name, '_rel_', vax_timing_lab, '_', gsub("-","_", age_group_tmp), loc_subname, '.png'))
  print(filename_)
  
  png(filename = filename_, 
      width = 1300, height = 1000, units = 'px', pointsize = 14.5)
  
  options(repr.plot.width=100, repr.plot.height=125)
  par(mfrow=c(3,2))
  ...
  for (comparison in c("All booster vs Counterfactual", "High-risk booster vs Counterfactual", "All vs High-risk booster")) {
    ...
    for (tgt in tgt_vec) {
      ...
      mdls[[vax_timing]][[tgt]][[comparison]] <- meta
      # if (imm_esc == "Low"){
      figs[[vax_timing]][[tgt]][[comparison]] <- invisible(metafor::forest(meta,
                                                                           digits=3,
                                                                           col=outcome_colors[outcome_col_i],
                                                                           cex=cex_val,
                                                                           header=T,
                                                                           refline=refline,
                                                                           xlab=sprintf("%s (Relative) - %s, %s Immunization%s",
                                                                                        tgt, comparison, vax_timing, age_text_snip)))
      # }
  ...
  for (comparison in c("All booster vs Counterfactual", "High-risk booster vs Counterfactual", "All vs High-risk booster")) {
    ...
    for (tgt in tgt_vec) {
      ...
      figs[[vax_timing]][[tgt]][[comparison]] <- invisible(metafor::forest(meta,
                                                                           ...
                                                                           header=T,
                                                                           refline=refline,
                                                                           xlab=sprintf("%s (Relative) - %s, %s Immunization",
                                                                                        tgt, comparison, vax_timing)))
      dev.off()
      # }
      meta_res_rel <- meta_res_rel %>% bind_rows(
        tibble(comp = comparison, vax_tmg = vax_timing, target = tgt,
...
meta_res_rel %>% 
  mutate(vax_tmg=case_when(vax_tmg==c("Classic Immunization")~c("Classic timing"), 
                           vax_tmg==c("Early Immunization")~c("Early timing"))) -> meta_res_rel_plot
...
# x.grob <- textGrob("              Cumulative percent prevented by increased vaccination,\n              April 27, 2025 to April 25, 2026",
#                    gp=gpar(fontsize=18))
# 
# y.grob <- textGrob("Vaccination target comparison", 
#                    gp=gpar(fontsize=18), rot=90)
...
# 
# png(filename = file.path(plot_dir, 
#                          paste0(meta_name, '_rel_pooled_', gsub("-","_", age_group_tmp), loc_subname, '.png')),
#     width = 12,
#     height = 6,
#     units = 'in',
#     res = 600)
...
meta_res_rel_plot$comp <- factor(meta_res_rel_plot$comp, levels=c("All vs High-risk booster", "High-risk booster vs Counterfactual", "All booster vs Counterfactual"))
meta_res_rel_plot$vax_tmg <- factor(meta_res_rel_plot$vax_tmg, levels=c("Classic timing", "Early timing"))

rel_vaccine_impact <- meta_res_rel_plot %>% mutate(table=paste0(format(round((1-mean)*100,0), nsmall=0),"%"," (",
                                                                format(round((1-ub)*100,0), nsmall=0),"-",
                                                                format(round((1-lb)*100,0), nsmall=0),"%",")")) %>%
  ggplot(aes(y=comp))+
  facet_wrap(~target, nrow = 2)+
  geom_errorbarh(aes(xmin = 1-lb, xmax = 1-ub, group=vax_tmg, color=vax_tmg), 
                 stat="identity", position = position_dodge(width = 0.75), height = .2) +
  geom_point(aes(x = 1-mean, group=vax_tmg, color=vax_tmg), 
             stat="identity", position = position_dodge(width = 0.75), size = 3, pch = 16) +
  geom_text(aes(x = 1-mean, label = table, group = vax_tmg),
            stat="identity", position = position_dodge(width = 1), vjust = -.6, size = 4) +
  scale_color_manual("Immunization timing", values=c("#1380A1", "#990000")) +
  guides(color = guide_legend(reverse=TRUE)) +
  theme(text = element_text(size=12, family="sans",color="black"),
        axis.text = element_text(size=13, family="sans",color="black"),
        plot.title = element_text(size=16, family="sans",color="black", hjust=0.5),
        strip.text = element_text(size = 14, family="sans",color="black"),
  ...
  labs(y="Vaccination scenario comparison \n", 
       ...
       title=paste0("Percent prevented (95% CI)", ifelse(age_group_tmp=="0-130", "", paste0("\n  among those aged ", age_group_label))))
...
table_total <- meta_res_abs_plot %>% 
  mutate(table=case_when(vax_tmg==c("Early timing") ~ 
                           paste0("Early: ", convert_to_K(mean)," (",convert_to_K(lb),"-",convert_to_K(ub),")"),
                         vax_tmg==c("Classic timing") ~ 
                           paste0("Classic: ", convert_to_K(mean)," (",convert_to_K(lb),"-",convert_to_K(ub),")")))

table_total$target <- factor(table_total$target, levels=c("Hospitalizations", "Deaths"))
table_total$comp <- factor(table_total$comp, levels=c("All vs High-risk booster", "High-risk booster vs Counterfactual", "All booster vs Counterfactual"))
table_total$vax_tmg <- factor(table_total$vax_tmg, levels=c("Classic timing", "Early timing"))
...
table_total %>% mutate(mean=0.1) %>%
  ...
  geom_point(aes(x = 1-mean, group=vax_tmg, color=vax_tmg), 
             stat="identity", position = position_dodge(width = 0.75), size = 3, pch = 16) +
  geom_text(aes(x = 1-mean, label = table, group = vax_tmg),
            stat="identity", position = position_dodge(width = 0.75), size = 4) +
  scale_color_manual("Immunization Timing", values=c("#FFFFFF", "#FFFFFF")) +
  guides(color = guide_legend(reverse=TRUE)) +
  theme(text = element_text(size=12, family="sans",color="black"),
        axis.text = element_text(size=13, family="sans",color="black"),
        plot.title = element_text(size=16, family="sans",color="black", hjust=0.5),
  ...
  labs(y="", x="\n Cumulative difference between scenarios, \n April 27, 2025 to April 25, 2026", 
       title=paste0("Total prevented (95% CI)", ifelse(age_group_tmp=="0-130", "", paste0("\n  among those aged ", age_group_label)))) -> rel_vaccine_impact_table



options(repr.plot.width=14,repr.plot.height=10.5)
Figure <- cowplot::plot_grid(rel_vaccine_impact, rel_vaccine_impact_table, align = "h", nrow=1,
                             rel_widths = c(2, 1))

title <- textGrob(paste0("Individuals aged ", age_group_label), gp=gpar(fontsize=18), hjust = 1.62)

# ggsave(file.path(plot_dir, 
#                          paste0("combined_", gsub("-","_", age_group_tmp), loc_subname, ".png")), Figure, width=14, height=10.5, bg='white', dpi=300)

ggsave(file.path(plot_dir, 
                 paste0("combined_", gsub("-","_", age_group_tmp), loc_subname, ".png")), Figure, width=14, height=panel_height, bg='white', dpi=300)




## simple figure only for classic timing
meta_res_rel_plot %>% mutate(table=paste0(format(round((1-mean)*100,0), nsmall=0),"%"," (",
                                          format(round((1-ub)*100,0), nsmall=0),"-",
                                          format(round((1-lb)*100,0), nsmall=0),"%",")")) %>%
  filter(vax_tmg == "Classic timing") %>%
  ggplot(aes(y=comp))+
  facet_wrap(~target, nrow = 2)+
  geom_errorbarh(aes(xmin = 1-lb, xmax = 1-ub), color= "grey20",
                 stat="identity", position = position_dodge(width = 0.9), height = .2) +
  geom_point(aes(x = 1-mean, group=vax_tmg), color= "grey20",
             stat="identity", position = position_dodge(width = 0.9), size = 3, pch = 16) +
  geom_text(aes(x = 1-mean, label = table),
  ...
  theme(text = element_text(size=15, family="sans",color="black"),
        ...
        panel.spacing = unit(2, "lines"),
        legend.position = c(0.75, 0.61), legend.background = element_rect(fill = "white", colour = NA)) +
  scale_x_continuous(breaks=c(0, .1, .2, .3, .4), labels=c("0%", "10%", "20%", "30%", "40%")) +
  # labs(y="Vaccination scenario comparison \n", x="", title="Percent prevented (95% CI) \n")
  labs(y="Vaccination scenario comparison \n", 
       x=paste0("Percent prevented (95% CI)", ifelse(age_group_tmp=="0-130", "", paste0("\n  among those aged ", age_group_label)),
                "\n April 27, 2025 to April 25, 2026")) +
  ggtitle("Classic timing immunization") -> rel_vaccine_impact

meta_res_abs_plot %>% filter(vax_tmg==c("Classic timing")) %>%
  mutate(table=paste0(convert_to_K(mean)," (",convert_to_K(lb),"-",convert_to_K(ub),")")) -> table_total
...
table_total %>% mutate(mean=0.1) %>%
  ggplot(aes(y=comp))+
  facet_wrap(~target, nrow = 2)+
  geom_point(aes(x = 1-mean, group=vax_tmg),  color= "#FFFFFF",
  ...
  theme(text = element_text(size=15, family="sans",color="black"),
        ...
        axis.text.y=element_blank(), axis.ticks.y=element_blank(), panel.grid.major=element_blank()) +
  scale_x_continuous(breaks=c(0, .1, .2, .3, .4), labels=c("0%", "10%", "20%", "30%", "40%"))+
  labs(y="", x=paste0("Total prevented (95% CI)", ifelse(age_group_tmp=="0-130", "", paste0("\n  among those aged ", age_group_label)),
                      "\n April 27, 2025 to April 25, 2026")) -> rel_vaccine_impact_table

options(repr.plot.width=12,repr.plot.height=8)
cowplot::plot_grid(rel_vaccine_impact, rel_vaccine_impact_table, align = "h", nrow=1,
                   rel_widths = c(2, 0.8)) -> Figure

numdiff_for_text <- numdiff_for_text %>% bind_rows(table_total %>% mutate(location = loc_abbr, age = age_group_label, axis_comp = "vacc"))
reldiff_for_text <- reldiff_for_text %>% bind_rows(meta_res_rel_plot %>% 
                                                     mutate(table = paste0(format(round((1-mean)*100,0), nsmall=0),"%"," (",
                                                                           format(round((1-ub)*100,0), nsmall=0),"-",
                                                                           format(round((1-lb)*100,0), nsmall=0),"%",")"))  %>% mutate(location = loc_abbr, age = age_group_label, axis_comp = "vacc"))

ggsave(file.path(plot_dir, 
                 paste0("combined_simple_", gsub("-","_", age_group_tmp), loc_subname, ".png")), Figure, width=14, height=7, bg='white', dpi=300)
